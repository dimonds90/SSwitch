...
typedef ISwitcherIPP** (WINAPI *PGetInstanceList)();
...
{
	{
		...
		if (SUCCEEDED(hr) && spSettings)
		{
			...
			{
				VARIANT_BOOL ok = 0;
				spSettings2->requestMediaAccessRights(L"full", &ok);
			}
		}


		HMODULE hModule = GetModuleHandle("MMSwitch.ax");
		PGetInstanceList pGetInstanceList = (PGetInstanceList)GetProcAddress(hModule, "GetInstanceList");

		if (!pGetInstanceList)
			return;

		ISwitcherIPP** pISwitcherList = pGetInstanceList();

		CComBSTR bstrSwitcherURL;
		ISwitcherIPP* pISwitcher = NULL;
		int i;
		for (i = 0; i < 256; i++)
		{
			pISwitcher = pISwitcherList[i];
			if (pISwitcher && (S_OK == pISwitcher->get_sourceURL(&bstrSwitcherURL)))
			{
				if (lstrcmpW(bstrSourceURL, bstrSwitcherURL) == 0)
				{
					pISwitcher->AddRef();
					break; // for
				}
		...
		char Name[MAX_PATH];
		if (pISwitcher->GetName(1, Name) != S_OK)
		{
			pISwitcher->Release();
			return;
		}
		...
		if (SUCCEEDED(hr) && spControls)
		{
			...
			if (SUCCEEDED(hr) && spControls3)
			{
				...
				{
					long lNewIndex = pISwitcher->GetSelected() + 1;
					if (lIndex != lNewIndex)
